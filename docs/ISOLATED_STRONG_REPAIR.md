...
默认使用[先草案、后集中修复](DRAFT_THEN_REPAIR.md)。17 步草案结束后，最终工程修复使用所选强模型；遇到路由所属缺陷时，隔离执行器从首次修复入口接手工程副本：观察实际几何和 CAD 图片，编写 Python 修改副本，由宿主重新运行真实 KiCad 校验，改善后提交。关闭草案策略时仍保留原来的连续失败升级入口。
