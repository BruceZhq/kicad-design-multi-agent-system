...
async def _event_stream(
    ...
    async with serialize_thread_run(agent_id, client_thread_id):
        legacy_state = await graph.aget_state(
            config=RunnableConfig(
                configurable={"thread_id": client_thread_id}
            )
        )
        checkpoint_thread_id = (
            client_thread_id
            if legacy_state.values
            else f"{agent_id}:{client_thread_id}"
        )
        input_data = input_data.model_copy(
            update={"thread_id": checkpoint_thread_id}
        )
        configurable = dict(config.get("configurable", {}))
        configurable["client_thread_id"] = client_thread_id
        config = RunnableConfig(**{**config, "configurable": configurable})
