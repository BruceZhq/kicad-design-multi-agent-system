...
class VerifiedPinAlias(ContractModel):
    """One evidence-backed functional alias for a real library pin.

    The Architect may establish that a symbol pin named ``PA14`` also carries
    a datasheet function such as ``BOOT0``. Downstream gates consume only this
    structured record, never prose or an LLM-proposed net label. The real
    symbol name and number are retained so every consumer can revalidate the
    alias against the installed library before using it.
    """

    symbol_lib_id: str = Field(min_length=3, max_length=200)
    pin_number: str = Field(min_length=1, max_length=32)
    symbol_pin_name: str = Field(min_length=1, max_length=120)
    ...
    def _normalize_aliases(cls, value: list[str]) -> list[str]:
        ...
        if not normalized:
            raise ValueError("verified pin aliases must not be empty")
    ...
    def _normalize_evidence_ids(cls, value: list[str]) -> list[str]:
        ...
        if not normalized:
            raise ValueError("verified pin aliases require source evidence")
        return list(dict.fromkeys(normalized))
