...
human-readable Markdown review, optionally with an LLM narrative and finding
triage.
...
class Reviewer:
    def review(
        ...
        try:
            ...
            if mode == LlmMode.REQUIRED:
                raise LlmError(f"required LLM review failed: {exc}") from exc
            return ReviewResult(
                report=report,
                narrative=_deterministic_narrative(report),
                triage=_deterministic_triage(findings),
    ...
    def _live(
        ...
        return ReviewResult(report=report, narrative=narrative, triage=triage, source="llm")
