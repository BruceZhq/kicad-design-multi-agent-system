...
class AgentClient:
            ...
            match parsed["type"]:
                ...
                case "error":
                    ...
                    return ChatMessage(
                        ...
                        content=error_msg,
                        custom_data={
                            "code": parsed.get("code", "stream_error"),
    ...
    @staticmethod
    def _stream_event_id(line: str) -> int | None:
        ...
        for attempt in range(self.stream_retries + 1):
            ...
            try:
                ...
                with httpx.stream(
                    ...
                    for line in response.iter_lines():
                        ...
                        if stripped.startswith("id:"):
                            last_event_id = int(stripped[3:].strip())
                            continue
                        if not stripped.startswith("data: "):
                            continue
                        event_id = self._stream_event_id(stripped)
                        if event_id is not None:
                            last_event_id = event_id
                        parsed = self._parse_stream_line(stripped)
                        if parsed is None:
                            completed = True
                            break
                        yield parsed
                ...
                if attempt >= self.stream_retries:
                    raise AgentClientError(
                        f"Stream disconnected after {attempt + 1} attempts: {e}"
                    ) from e
            if attempt < self.stream_retries:
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(
                    f"{self.base_url}/runs/{request_id}",
                    params=params,
                    headers=self._headers,
                    timeout=self.timeout,
                )
