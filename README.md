...
### 从需求输入到 Release-ready：完整流程

橙色为新增或增强模块，蓝色为已有链路，绿色为发布终态。强模型升级为可选能力；所有候选修改仍须经过真实工程工具验证。
...
本地 evaluator 以只读方式挂载仓库，在独立 volume 建立 detached worktree；这是开发工作区隔离，不是恶意代码安全边界。生产 overlay 使用无 Secret、无网络、无 ServiceAccount Token 的一次性候选 Job，sealed 来源从不可变 evaluator 镜像执行。通用 Python 回归仍属于工作区隔离，不宣称密码学盲测。对新增生成器补丁，可信父进程不导入候选模块：候选子进程只通过实际 CAD 文件交付结果，再由独立 KiCad CLI/Python 验证引脚连接、线宽、ERC/DRC 和路由。此证据只批准进入人工治理的候选流程；正式晋级还要求已注册构建、固定版本端点和服务端真实灰度发布记录，不能靠候选自报成绩直接升级生产。
