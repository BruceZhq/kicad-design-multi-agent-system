...
public class EvolutionController {
    ...
    public EvolutionController(EvolutionService evolution) {
    ...
    List<EvolutionObservation> observations(
        ...
        return evolution.observations(tenantId, AuthenticatedActor.from(jwt));
    }
    ...
    List<CandidateResponse> candidates(
            @RequestHeader(ORGANIZATION_HEADER) UUID tenantId,
            @AuthenticationPrincipal Jwt jwt) {
        return evolution.candidates(tenantId, AuthenticatedActor.from(jwt)).stream()
    ...
    CandidateResponse candidate(
        ...
        return CandidateResponse.from(
                evolution.candidate(tenantId, candidateId, AuthenticatedActor.from(jwt)));
    }
    ...
    List<EvolutionTrial> trials(
            ...
            @PathVariable @Pattern(regexp = "[0-9a-f]{64}") String candidateId,
            @AuthenticationPrincipal Jwt jwt) {
        return evolution.trials(tenantId, candidateId, AuthenticatedActor.from(jwt));
    }
    ...
    CandidateResponse transition(
            ...
            @AuthenticationPrincipal Jwt jwt) {
        return CandidateResponse.from(evolution.transition(
                tenantId,
                candidateId,
