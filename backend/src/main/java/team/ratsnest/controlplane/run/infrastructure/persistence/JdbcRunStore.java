...
import team.ratsnest.controlplane.run.domain.port.RunStore.ReconciliationClaim;
...
public class JdbcRunStore implements RunStore {
    ...
    private static final String COLUMNS = """
            tenant_id, run_id, project_id, root_run_id, parent_run_id, forked_from_run_id,
            revision_number,
    ...
    public JdbcRunStore(JdbcClient jdbcClient, ObjectMapper objectMapper) {
        this.jdbcClient = jdbcClient;
        this.objectMapper = objectMapper;
    }

    public void insert(Run run, AuthenticatedActor actor) {
        jdbcClient.sql("""
                        insert into control_plane.runs (
                            tenant_id, run_id, project_id, root_run_id, parent_run_id,
                            forked_from_run_id, revision_number,
                            thread_id,
                            ...
                            :tenantId, :runId, :projectId, :rootRunId, :parentRunId,
                            :forkedFromRunId, :revisionNumber,
                            :threadId,
                ...
                .param("forkedFromRunId", run.forkedFromRunId())
    ...
    public Optional<Run> find(UUID tenantId, UUID runId) {
    ...
    public Optional<Run> findForUpdate(UUID tenantId, UUID runId) {
        return jdbcClient.sql("select " + COLUMNS + " from control_plane.runs "
    ...
    public Optional<Run> findByIdempotency(UUID tenantId, UUID projectId, String key) {
        return jdbcClient.sql("select " + COLUMNS + " from control_plane.runs "
    ...
    public int nextRevisionNumber(UUID tenantId, UUID rootRunId) {
    ...
    public Optional<Run> findLatestRevision(UUID tenantId, UUID rootRunId) {
        return jdbcClient.sql("select " + COLUMNS + " from control_plane.runs "
                        + "where tenant_id = :tenantId and root_run_id = :rootRunId "
    ...
    public List<Run> findRevisionChainThrough(
            UUID tenantId,
            UUID rootRunId,
            int revisionNumber) {
        return jdbcClient.sql("select " + COLUMNS + " from control_plane.runs "
    ...
    public Optional<Run> findLatestForThread(UUID tenantId, UUID projectId, String threadId) {
        return jdbcClient.sql("select " + COLUMNS + " from control_plane.runs "
                        + "where tenant_id = :tenantId and project_id = :projectId "
    ...
    public List<ConversationSummary> listConversations(UUID tenantId, UUID projectId, int limit) {
        return jdbcClient.sql("""
                        with ranked as (
                            select r.*,
                                   row_number() over (
    ...
    public void setDeliveryStatus(UUID tenantId, UUID runId, DeliveryStatus status) {
    ...
    public boolean updateFromRuntime(UUID tenantId, UUID runId, RuntimeRun runtime) {
        return jdbcClient.sql("""
                        update control_plane.runs
                        set state = :state,
    ...
    public boolean markWaitingForInput(UUID tenantId, UUID runId) {
        return jdbcClient.sql("""
                        update control_plane.runs
    ...
    public boolean markFailed(UUID tenantId, UUID runId, String code, String error) {
        return jdbcClient.sql("""
                        update control_plane.runs
    ...
    public List<ReconciliationClaim> claimForReconciliation(String workerId, int batchSize) {
        return jdbcClient.sql(
    ...
    public boolean releaseReconciliation(
    ...
    private Run map(java.sql.ResultSet resultSet, int rowNumber) throws java.sql.SQLException {
        return new Run(
                ...
                resultSet.getObject("forked_from_run_id", UUID.class),
                resultSet.getInt("revision_number"),
                resultSet.getString("thread_id"),
