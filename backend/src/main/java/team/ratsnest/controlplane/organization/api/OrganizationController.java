package team.ratsnest.controlplane.organization.api;
...
import team.ratsnest.controlplane.organization.application.OrganizationService;
...
public class OrganizationController {
    ...
    OrganizationResponse create(
        ...
        return OrganizationResponse.from(
                organizations.create(request.name(), JwtIdentity.from(jwt)));
    }

    @GetMapping
    List<OrganizationResponse> list(@AuthenticationPrincipal Jwt jwt) {
        return organizations.list(JwtIdentity.from(jwt)).stream()
    ...
    OrganizationResponse current(
            ...
            @AuthenticationPrincipal Jwt jwt) {
        return OrganizationResponse.from(
                organizations.get(tenantId, JwtIdentity.from(jwt)));
    }
